...
get_shortest_path <- function(from_node, to_node, graph, nodes, edges, weight_name) {
    ...
    # work around to not use the st_nearest_feature function
    dist <- as.data.frame(distm(df[,c("lon","lat")], as.data.frame(nodes)[,c("lon","lat")], fun=distHaversine))
    names(dist) <- as.data.frame(nodes)$nodeID
    nearest <- as.numeric(colnames(dist)[unlist(apply(dist, 1, which.min))])
